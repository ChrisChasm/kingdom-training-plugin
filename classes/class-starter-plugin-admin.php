...
/**
 * Kingdom_Training_Admin Class
 *
 * @class Kingdom_Training_Admin
 * @version	1.0.0
 * @since 1.0.0
 * @package	Kingdom_Training
 * @author Jeffikus
 */
final class Kingdom_Training_Admin {
	/**
	 * Kingdom_Training_Admin The single instance of Kingdom_Training_Admin.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	/**
	 * Main Kingdom_Training_Admin Instance
	 *
	 * Ensures only one instance of Kingdom_Training_Admin is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	...
	public function register_settings_screen () {
		$this->_hook = add_submenu_page( 'options-general.php', __( 'Kingdom Training Settings', 'kingdom-training' ), __( 'Kingdom Training', 'kingdom-training' ), 'manage_options', 'kingdom-training', array( $this, 'settings_screen' ) );
	...
	public function settings_screen () {
		global $title;
		$sections = Kingdom_Training()->settings->get_settings_sections();
	...
	public function render_settings ( $args ) {
		$token = $args['id'];
		$fields = Kingdom_Training()->settings->get_settings_fields( $token );
		...
		if ( 0 < count( $fields ) ) {
			foreach ( $fields as $k => $v ) {
				...
				add_settings_field( $k, $v['name'], array( Kingdom_Training()->settings, 'render_field' ), 'starter-plugin-' . $token , $v['section'], $args );
			}
		}
	} // End render_settings()
	...
	public function validate_settings ( $input ) {
		$sections = Kingdom_Training()->settings->get_settings_sections();
		$tab = $this->_get_current_tab( $sections );
		return Kingdom_Training()->settings->validate_settings( $input, $tab );
