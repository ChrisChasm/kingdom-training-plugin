 ...
 *
 * @package WordPress
 * @subpackage Kingdom_Training
 * @category Plugin
 * @author Matty
 * @since 1.0.0
 */
class Kingdom_Training_Post_Type {
	/**
	 * The post type token.
	 * @access public
	 * @since  1.0.0
	...
	public function register_taxonomy () {
		$this->taxonomies['thing-category'] = new Kingdom_Training_Taxonomy(); // Leave arguments empty, to use the default arguments.
	...
	public function meta_box_content () {
		...
		$html .= '<input type="hidden" name="starter-plugin_' . $this->post_type . '_noonce" id="starter-plugin_' . $this->post_type . '_noonce" value="' . wp_create_nonce( plugin_basename( dirname( Kingdom_Training()->plugin_path ) ) ) . '" />';
	...
	public function meta_box_save ( $post_id ) {
		...
		// Verify
		if ( ( get_post_type() != $this->post_type ) || ! wp_verify_nonce( $_POST['starter-plugin_' . $this->post_type . '_noonce'], plugin_basename( dirname( Kingdom_Training()->plugin_path ) ) ) ) {
			return $post_id;
		}
